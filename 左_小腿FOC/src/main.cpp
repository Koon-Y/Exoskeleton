...
void loop() {
  ...
  else if(mode == ASSIST)
  {
    ...
    if (current_emg_val > threshold) {
        ...
        if (effort > 1.0f) effort = 1.0f; // 限制最多就是 100%
        // 照比例給予負向推力 (出力越大，推力越大)
   ...
   // =====================================================
    ...
    // 【關鍵修改】：把發力的跟隨速度調到超高！
    float ramp_up_rate = 10.0f;   // 原本是 5.0。調到 25.0 代表幾乎瞬間跟上你的肌肉變化
    float ramp_down_rate = 3.0f;  // 收力稍微慢一點點，保持放下的安全感
    float max_step;
    // 判斷是正在「增加推力」還是「收回推力」
    ...
    // =====================================================
    // 3. 角度限制安全鎖 (Angle Limit & Directional Cutoff)
    // =====================================================
    float current_angle = motor.shaft_angle - angle_offset;
    float ANGLE_UP = -8.0f;     // 負向極限 (舉到最高點)
    float ANGLE_DOWN = 0.0f;   // 正向極限 (放到最低點)
    // 緩衝區設為 1.0 弧度
    // 意思是：-7 到 -8 是「舉起緩衝區」，-2 到 -1 是「放下緩衝區」
    float buffer_zone_up = 1.0f;    // 舉起時力道大，維持 1.0 的厚軟墊
    float buffer_zone_down = 0.5f;  // 放下時力道小，改成 0.3 的薄軟墊 (提早讓它掉下來)
    // --- 處理最高點 (往負向 -8 靠近) 緩衝 ---
    if (current_angle < (ANGLE_UP + buffer_zone_up) && t_cmd < 0.0f) {
        if (current_angle <= ANGLE_UP) {
            t_cmd = 0.0f;
        } else {
            // 假設 current_angle 是 -7.5
            // scale = (-7.5 - (-8.0)) / 1.0 = 0.5 (推力剩一半)
            float scale = (current_angle - ANGLE_UP) / buffer_zone_up;
            t_cmd *= scale;
        }
    }
    // --- 處理最低點 (往正向 -1 靠近) 緩衝 ---
        ...
        if (current_angle >= ANGLE_DOWN) {
            ...
            // 假設 current_angle 是 -1.2
            // scale = (-1.0 - (-1.2)) / 1.0 = 0.2 (推力剩 20%)
            float scale = (ANGLE_DOWN - current_angle) / buffer_zone_down;
            t_cmd *= scale;
        }
    }
    // =====================================================
